...
public interface IExecutionEventStore
{
    ...
    /// <summary>Retrieves all execution events for a run in monotonic order.</summary>
    IReadOnlyList<ExecutionEvent> GetRunEvents(string runId);

    /// <summary>Retrieves execution events filtered by category for a session.</summary>
    IReadOnlyList<ExecutionEvent> GetEventsByCategory(string sessionId, ExecutionEventCategory category);
...
public sealed class InMemoryExecutionEventStore : IExecutionEventStore
{
    ...
    private readonly ConcurrentDictionary<string, List<ExecutionEvent>> _runEvents = new();
    ...
    public void RecordEvent(ExecutionEvent evt)
    {
        ...
        lock (_lock)
        {
            ...
            {
                var list = _runEvents.GetOrAdd(evt.RunId, _ => new List<ExecutionEvent>());
                list.Add(evt);
            }
        }
    ...
    public IReadOnlyList<ExecutionEvent> GetRunEvents(string runId)
    {
        if (string.IsNullOrEmpty(runId)) return Array.Empty<ExecutionEvent>();

        lock (_lock)
        {
            if (_runEvents.TryGetValue(runId, out var list))
            {
                return list.OrderBy(e => e.SequenceNumber).ToList();
            }
        }
    ...
    public IReadOnlyList<ExecutionEvent> GetEventsByCategory(string sessionId, ExecutionEventCategory category)
    {
        if (string.IsNullOrEmpty(sessionId)) return Array.Empty<ExecutionEvent>();
        ...
        lock (_lock)
        {
            if (_sessionEvents.TryGetValue(sessionId, out var list))
            {
                return list.Where(e => e.Category == category).OrderBy(e => e.SequenceNumber).ToList();
            }
        }
        return Array.Empty<ExecutionEvent>();
    }
