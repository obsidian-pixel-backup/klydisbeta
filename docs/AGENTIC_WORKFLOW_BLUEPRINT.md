...
| 20 | Model-owned state architecture wiring (2026-08, blueprint consolidation): (a) **killed the parser-generated-plan path** — verified `InitialPlanGenerator` is never instantiated, `TaskDecomposer.Decompose` feeds only the dead generator + tests, and `SeedSessionPlanAsync` is a no-op: the model owns planning exclusively through the structured `plan` tool (the split-line parser was restored last round, so the tool actually creates items again); (b) **wired `AgentContextSnapshot`** — the scaffolded compact state projection is now BUILT from live runtime state (`AgentContextSnapshotBuilder`: objective + plan summary + workspace + recent evidence + current step) and injected as a small `[AGENT STATE]` block in autonomous goal prompts, so the model gets a compact projection instead of re-serialized execution history (blueprint §8/§10/§28/§53); (c) **wired `PlanningPolicy`** — the planning directive now routes through `PlanningPolicy.Evaluate`, so a trivial autonomous request ("what is the CPU usage?") gets a "planning not required — execute directly" directive instead of being forced through a plan ceremony, while multi-step/build work stays Required (blueprint §21/§22); (d) still-scaffolded-but-unwired (next milestones): `AgentTodo` + `todo.*` tools + evidence accumulation, the canonical event journal → right-panel projections, and per-task execution-history persistence | ✅ Implemented (a–c) |
| 19 | smeagle rerun round (2026-08, from `CPU Diagnostic Report` rerun exports + review of the smeagle diagnosis): (a) **restored the gutted `plan` tool** — `SplitPlanItems` had been stubbed to `Enumerable.Empty` (a prior change), so `plan action=create/add` created ZERO items: the right panel stayed empty, the completion gate could never open, and plan-silent models could never self-finish (this is the real root of the "right panel not updating / plan not tracking" reports — 5 plan tests were failing and now pass after the restore); (b) **run_command replay exemption for read-only commands** — the replay guard now classifies `run_command` by command TEXT (positive allowlist: `Get-*`, `systeminfo`, `tasklist`, `wmic`, `git status`, ... with a mutating-marker scan + `>` redirect check), so an identical read-only diagnostic re-run is allowed (fresh reading) instead of hard-blocked — the observed 9× `REPLAY_DETECTED` loop on a read-only command; mutating commands stay blocked and can never be misclassified as safe; (c) alias `system_process` → `system_processes` (the rerun's one invented tool); (d) **single-layer telemetry accounting** — exports now report `Tool calls (proposed)` / `Tool executions` / `Tool successes` / `Tool failures` / `Tool rejections` from ONE authoritative event layer with a reconciliation note, so a mixed-layer "26 calls / 13 successes / 16 failures" can never recur; `AgentQualityTelemetry` no longer folds gate rejections into physical failures; (e) **Skills counter fixed** — the dossier now counts `SkillInvocationCompleted` (what ChatViewModel actually emits), so a run that DID select skills no longer reports "Skills: 0" | ✅ Implemented |
| 21 | Model-owned plan + TODO tools (2026-08, blueprint Phase 2 + Phase 3): (a) **structured plan tools exposed to the model** — `plan.create` / `plan.add_task` / `plan.update_task` / `plan.remove_task` / `plan.complete_task` / `plan.block_task` / `plan.replan`, each backed by a session-scoped `PlanEngine` instance (the rich ExecutionPlan model — objective/strategy/typed tasks with dependencies/capabilities/outputs/verification/completion — was previously used NOWHERE in the runtime; it is now the authoritative plan source, with every mutation validated by `PlanValidator`, revisioned, then projected back onto the flat checklist so the UI/plan-gate/completion consumers keep working unchanged); `plan.replan` preserves completed-task history in immutable revisions; (b) **structured TODO tools** — `todo.create` / `todo.update` / `todo.complete` / `todo.block` / `todo.reopen` / `todo.list` backed by a new `AgentTodoManager` + a durable `agent_todos` SQLite table (status lifecycle pending→running→completed/blocked/failed, plan-task linkage, related files, expected outputs, verification, lifecycle timestamps, and model-reported evidence on complete) — persisted per session so TODO state survives restarts; (c) **legacy `plan` tool kept in sync** — after the flat `plan` tool mutates the checklist, the structured `PlanEngine` view is rebuilt from it, so the two paradigms never diverge; (d) 13 new regression tests (structured plan lifecycle, checklist sync, legacy↔structured coherence, TODO lifecycle + persistence across manager instances); 1299 tests pass, App 0 warnings | ✅ Implemented |
